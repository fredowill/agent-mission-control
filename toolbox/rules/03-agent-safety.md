...
Before killing any claude.exe PID, check what session it belongs to (look up its state file in `states/` in the MC repo for mission/topic context) and confirm with the user: "PID X is running session Y which is working on Z -- should I kill it?" Process start time alone is NOT a reliable signal. A session started hours ago can still be actively working.
